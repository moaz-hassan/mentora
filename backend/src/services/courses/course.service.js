...
export const getCourseById = async (courseId) => {
  const course = await Course.findByPk(courseId, {
    attributes: ["id", "title", "description", "category", "level", "price", "thumbnail_url", "status", "intro_video_url", "intro_video_public_id", "intro_video_hls_url", "intro_video_duration"],
    include: [
      { model: Chapter, order: [["order_number", "ASC"]], include: [{ model: Lesson, order: [["order_number", "ASC"]] }, { model: Quiz, order: [["order_number", "ASC"]], attributes: ["id", "title", "order_number", "questions"] }] },
    ],
  });
      ...
      if (chapter.Lessons) {
        chapter.Lessons = chapter.Lessons.map(lesson => {
          if (lesson.is_preview) {
            return { id: lesson.id, title: lesson.title, lesson_type: lesson.lesson_type, video_url: lesson.video_url, video_public_id: lesson.video_public_id, hls_url: lesson.hls_url, content: lesson.content, duration: lesson.duration, order_number: lesson.order_number, is_preview: lesson.is_preview };
          }
          return { id: lesson.id, title: lesson.title, lesson_type: lesson.lesson_type, duration: lesson.duration, order_number: lesson.order_number, is_preview: lesson.is_preview };
        });
      }
      if (chapter.Quizzes) {
        chapter.Quizzes = chapter.Quizzes.map(quiz => ({ id: quiz.id, title: quiz.title, order_number: quiz.order_number, questions_length: quiz.questions.length }));
      }
      return chapter;
    });
  }
