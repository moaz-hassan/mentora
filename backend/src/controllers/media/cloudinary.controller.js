...
/**
 * Generate a secure Cloudinary upload signature for material uploads (documents, PDFs, etc.)
 * This allows frontend to upload directly to Cloudinary
 */
export const getMaterialUploadSignature = async (req, res, next) => {
  try {
    ...
    // Parameters to sign - MUST match exactly what's sent to Cloudinary
    // Note: resource_type is NOT included in signature, it's part of the URL
    const paramsToSign = {
      folder: folder,
      timestamp: timestamp,
    };
    ...
    res.status(200).json({
      ...
      data: {
        ...
        apiKey: process.env.CLOUDINARY_API_KEY,
        folder: folder,
        resourceType: 'raw', // Send separately, not in signature
      },
    });
  } catch (error) {
    next(error);
